...
function writeJson(path, json, indent) {
    ...
    return exports.writeFile(path, JSON.stringify(json, null, indent));
}
...
function transformJson(path, transform) {
    return transformFile(path, function (contents) {
        var indent = contents ? detectIndent(contents).indent : 2;
        var json = contents ? parseJson(contents, path) : undefined;
        return Promise.resolve(transform(json))
            .then(function (json) { return JSON.stringify(json, null, indent || 2); });
    });
}
...
function writeDependency(contents, options) {
